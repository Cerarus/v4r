...
int main(int argc, const char * argv[]) {
    ...
    desc.add_options()
            ...
            ("input,i", po::value<std::string>(&inputUsr)->required(), "either input path or input model (.ply)")
    ...
    if((inputUsr.compare(inputUsr.find(".")+1,3,"ply"))==0){
            InputV.push_back(inputUsr);
    }
    else{
            std::vector<std::string> Temp(v4r::io::getFilesInDirectory(inputUsr,".*.ply", false));
            for(size_t j=0;j<Temp.size();j++)
            InputV.push_back(Temp[j]);
    }

    for(size_t j=0;j<InputV.size();j++){
        if(InputV.size()>1){
            input = inputUsr;
            input.append(InputV[j]);
        }
        else
            input = inputUsr;

        out_dir = input.substr(0,input.find("."));
        out_dir.append("/");
        std::cout<<"Rendering File:"<<input.substr(input.find_last_of("/")+1,input.find("."))<<std::endl;
